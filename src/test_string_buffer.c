...
// `string_buffer_append_len`'s length guard exists for rather than for speed. Without the guard the
// call reaches `memcpy(dst, NULL, 0)`, which is undefined even for a zero length and which no
// sanitizer here reports.
...
// `src/string_buffer.c` and so not reachable from here). Raising that constant means raising this
// count, or the test stops forcing a growth. The test asserts the exact final capacity rather than
// a lower bound. Content surviving is true under any growth factor, so without the exact assertion
// neither the seed nor the doubling is pinned and a buffer that grew one byte at a time would pass.
...
// the contract in `string_buffer.h`. Three things rest on the reset: an append after the free
...
// The sanitizer build is configured to preserve `realloc`'s failure-return behavior rather than
// aborting on an allocation beyond ASan's supported size. A failed reserve leaves the buffer
// unchanged.
static void test_allocation_failure_is_returned(void) {
  struct StringBuffer buffer;
  string_buffer_init(&buffer);
  TEST_ASSERT(string_buffer_append(&buffer, "kept") == 0);

  TEST_CHECK(string_buffer_reserve(&buffer, SIZE_MAX / 4) == -1);
  TEST_CHECK(buffer.len == 4);
  TEST_CHECK(strcmp(buffer.data, "kept") == 0);
...
TEST_LIST = {
    ...
    {"allocation failure is returned", test_allocation_failure_is_returned},
    {NULL, NULL},
};
